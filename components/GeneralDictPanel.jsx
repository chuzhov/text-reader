...
export default function GeneralDictPanel({
  ...
  onRemoveWord, onAddToActive, onAddWord,
}) {
  ...
  async function handleAdd() {
    ...
    if (selectedSource === 'en' && /^to [^\s]+$/i.test(word)) word = word.slice(3);
    setAddLoading(true);
    setAddShake(false);
    const result = await onAddWord(word, selectedSource, selectedTarget);
    setAddLoading(false);
    if (result) {
      setShowAddBar(false);
      setAddWord('');
